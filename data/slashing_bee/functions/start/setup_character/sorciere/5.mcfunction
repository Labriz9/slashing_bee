...
scoreboard players set @e[tag=Witch_Cat,limit=1,sort=nearest] Cat_Level 3
schedule function slashing_bee:process/abilities/sorciere/witch_cat_loop 15s
tag @s add More_Money
# Ability
tag @s add Potion
tag @s add Potion_Up
item replace entity @s hotbar.2 with splash_potion{display:{Name:'[{"text":"Potion de Soin instantané Supérieure","italic":false,"color":"#0091FE"}]',Lore:['[{"text":"Compétence Rare","italic":false,"color":"dark_gray"}]','[{"text":""}]','[{"text":"Description :","italic":false,"color":"white"}]','[{"text":"Rend 8pv au maximum","italic":false,"color":"dark_green"}]']},HideFlags:32,CustomPotionColor:16711680,custom_potion_effects:[{id:instant_health,duration:1,amplifier:1}],Witch:1b}
